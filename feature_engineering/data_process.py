...
def featmap_gen(tmp_df=None):
    ...
    time_span = [
        ...
        129600,
        259200,
    ]  # Increase in the number of time windows to increase the characteristics.
...
if __name__ == "__main__":
    ...
    # dgl.save_graphs(DATADIR + "graph-yelp.bin", [g])
    ...
    # dgl.save_graphs(DATADIR + "graph-amazon.bin", [g])
    ...
    g.ndata["label"] = torch.from_numpy(labels.to_numpy()).to(torch.long)
    g.ndata["feat"] = torch.from_numpy(feat_data.to_numpy()).to(torch.float32)
    dgl.save_graphs(DATADIR + "graph-S-FFSD.bin", [g])
